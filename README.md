...
- **G-buffer / deferred path (WIP, M1)**: the opaque pass can store the surface instead of shading it - `pbr.vert` + `gbuffer.frag` write three single-sampled targets (albedo+metallic in `RGBA8`, world normal+roughness in `RGBA16F`, material id + ambient occlusion + material flags in `RGBA8`: 16 bytes per pixel, depth in the pass's own 1x depth image), and `pbr.frag`/`gbuffer.frag` gather that surface through one shared include (`shaders/surface.glsl`: material-table lookup, the glTF MASK discard, normal mapping, the double-sided flip), so the two paths cannot drift apart. `runtime::set_gbuffer_debug(true)` + `[render] gbuffer_debug` draws it and shows one channel through the ordinary post chain - `post.vert` + `gbuffer_debug.frag` read the three targets and the depth image and display albedo / world normal / roughness / metallic / AO / material id / linearized depth / raw flags (`set_gbuffer_channel`, overlay combo). The view exists because a G-buffer is the one part of the renderer whose contents cannot be judged from a shaded screenshot: a wrong normal encoding or a swapped roughness channel only looks like "the lighting is a bit off" once a later pass consumed it. alphaMode BLEND geometry stays in the forward transparent pass (a G-buffer cannot carry a blended surface), bloom is forced off while the view is up, and the forward path is untouched - the two share nothing but the vertex stage. This is the foundation the deferred lighting pass (M2) lands on.
- **Debug GUI (`vulkan.gui`)**: a Dear ImGui overlay driven inside the runtime's frame steps (**F1** shows/hides it at runtime without losing its panels; the key even initializes it on demand when `[gui] show` started false). `vulkan::gui::widget` subclasses (`label_widget` / `checkbox_widget` / `slider_widget` / `vec3_widget` / `combo_widget`...) stack into `debug_panel`s that register with `gui_content` via `runtime::debug_gui()`. The demo panel shows fps and the per-pass **GPU timing** line (see the feature above), toggles frustum culling, the skybox pass, the shadow pass, **FXAA** (checkbox + sub-pixel / edge-threshold sliders), the **G-buffer debug view** (checkbox + channel combo, see above), switches the **render mode** (`pbr` lit ↔ `unlit` flat base color — the runtime default pipeline, so every default-semantics leaf re-shades live, useful as a shading-free reference or for normal-less test assets), adjusts the shadow depth bias (slope / constant sliders), edits the camera orbit target, and drives two demo **point lights** (enable + position / color / intensity / range, pushed to the runtime once per frame via `chores::apply_point_lights`); animated models additionally get play/pause, a time scrubber and an animation dropdown. Window layout (position / size) persists to `imgui_layout.ini`.
