...
- **Cel/toon shading**: the `toon strength` (0 = plain PBR, 10 = strongest cartoon) / `toon softness` sliders quantize the direct-light diffuse falloff into 2..8 bands, harden the shadow edge and turn the specular lobe into a hard highlight block. Fewer bands = harder cel look (more bands converge back to smooth PBR), and a small softness keeps the band edges crisp; only the shading response is banded - geometry, silhouettes and the IBL ambient stay smooth. Only the shading response is banded - geometry, silhouettes and the IBL ambient stay smooth.
