...
- **Screenshots**: press **F12** - the runtime copies the presented swapchain image back to host memory (`runtime::acquire_current_frame_image()`, 8-bit RGBA) and the demo writes it as `screenshot_<timestamp>.png` via `utility::write_png()` (a dependency-free PNG encoder in the utility module).
