...
in a comment block at the top of its main interface unit — `utility` 0.6.0, `vstd` 0.1.0a
...
`vulkan.core` and `vulkan.runtime` embed), `vulkan.core` 0.7.0, `vulkan.runtime` 0.21.2,
`vulkan.profiling` 0.1.0, `vulkan.pipelines` 0.1.0, `vulkan.bindings` 0.4.0, `vulkan.shadow_fit` 0.1.0,
`vulkan.readback` 0.1.0,
...
- **Screen-space global illumination**: `shaders/ssgi.comp` traces one bounce of diffuse indirect light at **half resolution** - each invocation takes its pixel's stored normal and albedo, shoots `ssgi_rays` cosine-weighted rays, marches the G-buffer depth in screen space and averages the direct radiance it finds at the hits (`albedo * mean(radiance)`, where the pi factors cancel by construction). It samples the HDR scene target as it stands after the lighting stage, and that pass position is what keeps it stable: the target is rewritten earlier in every frame, so a hit can never return the tracer's own previous output - a screen-space GI that samples an image containing its own result has a loop gain above 1 and accumulates energy. Two rays per pixel is white noise with the right average, so the raw trace is not what the composite sees: `shaders/ssgi_temporal.comp` is a second compute pass that turns it into an image, reprojecting the previous frame's resolved result with the motion-vector target, dropping the history on a view-depth mismatch (the classic disocclusion), clamping it into the 3x3 neighbourhood of the raw trace and blending it in with the GI pass's own weights rather than TAA's. The clamp box is deliberately taken from the noisy current frame, so it is wide and rejects little - which is exactly what lets the accumulation average ten frames of differently-seeded rays per pixel; tightening belongs in a spatial filter after this one. A copy of the resolved image becomes the next frame's history, the same copy-not-ping-pong arrangement the TAA resolve uses. The result is **added to** the environment probe rather than replacing it, because a ray that leaves the frame hits nothing: the probe stays the off-screen fallback and `[render] ssgi_intensity` reconciles the two (they overlap light, so more is not better), while `ssgi_radius` is a fraction of the scene radius so one value means the same thing on a 1.6-unit model and on Sponza's 18.5. Verified on Sponza's interior view (1080x960, TAA off, 2 rays x 6 steps): with `ssgi = false` the captured frame is **byte-identical** to the pre-GI reference (the composite's GI weight is 0 and no GI pass is recorded); with it on at the default intensity the added light averages **+1.39/255** (442201 pixels brighter, 0 darker, worst 35), the resolve's own frame-to-frame residual is **0.43/255** (worst 18), and frame 20 against frame 40 differs by -0.03/255 mean - i.e. it converges instead of accumulating energy. The resolve's effect on the raw trace is measured at intensity 4.0, where an 8-bit capture has something to measure: the frame-to-frame difference drops from **5.11 to 2.05 mean** and the share of pixels moving by more than 8/255 from **18.7% to 5.7%** (worst 141 -> 70), and the 16x-amplified added-light images show the white noise gone with the arches, columns and ceiling ribs readable. Honest limits: off-screen geometry and thin occluders between two march steps contribute nothing (hence the addition to the probe), alphaMode MASK surfaces are solid because a depth buffer has no alpha - which is also what an inline ray query would see without any-hit shaders - and a deforming mesh has no motion vector, so its GI trails. Knobs: `[render] ssgi` / `ssgi_intensity` / `ssgi_radius` / `ssgi_rays` / `ssgi_steps`. No overlay controls: they are read once at startup, so changing them needs a restart.
...
Positional argv overrides the file: `argv[1]` = model path, `argv[2]` = grid side (a number). Configurable: model / instancing grid, `shaders_dir` / `model_dir` paths, window size / title / vsync / clear color, the initial camera framing (`[render] camera_fit`), shadow & FXAA stage toggles, GPU pass timings (`[render] gpu_timings`), the G-buffer debug view (`[render] gbuffer_debug` / `gbuffer_channel`), TAA (`[render] taa` / `taa_blend_static` / `taa_blend_min`), cascaded shadow maps (`[render] shadow_cascades` / `shadow_cascade_blend` / `shadow_map_size`), clustered light culling (`[render] clustered_lights` + `[lighting] demo_lights`), screen-space ambient occlusion (`[render] ssao` / `ssao_radius` / `ssao_intensity` / `ssao_samples`), screen-space global illumination (`[render] ssgi` / `ssgi_intensity` / `ssgi_radius` / `ssgi_rays` / `ssgi_steps`), the render mode (`[render] unlit`), IBL precompute resolutions, and the debug-panel default size.
