...
- **Cel/toon shading**: the `toon steps` / `toon softness` sliders quantize the direct-light diffuse falloff into bands, harden the shadow edge and turn the specular lobe into a hard highlight block (0 steps = plain PBR). Only the shading response is banded - geometry, silhouettes and the IBL ambient stay smooth.
