...
embed). All started at 0.1.0; `vulkan.core`, `vulkan.runtime`, `vulkan.animation` and
`vulkan.constant_init` are at 0.1.1.
...
- **Static/merged batching (`static_draw_primitive`)**: one primitive OWNS a merged vertex/index buffer and draws a chunk table over it — one buffer bind, then one offset draw per chunk with each chunk's own material. N static sub-meshes cost 1 bind + N draws instead of N binds + N draws; the same buffer can carry many different materials. Built via `runtime::make_static_draw(static_draw_create_info)` from already-merged CPU geometry (the packing itself stays at the call site); a lightweight `offset_draw_primitive` covers drawing a sub-range of another primitive's shared geometry.
