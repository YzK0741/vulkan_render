...
- **Directional shadows**: orthographic shadow pass rendering the scene's depth from the sun (2048² dynamic-rendering depth pass whose ortho box **follows the camera every frame** - the part of the view volume that can actually hold the scene (the camera's own far plane is deliberately generous, so the fit is clipped at `|eye - scene centre| + scene_radius`) sets the box's light-space xy extent while its depth range additionally covers every caster whose parallel shadow column can reach the view, and the box center is snapped to the texel grid while the box SIZE is rounded up to a whole number of texels (with one texel of margin) to stop the edges crawling - center snapping alone only fixes translation, a continuously changing size rescaled the map and made them crawl anyway, and the margin is what keeps the visible footprint off the map's edge; a large camera move still crosses a rounding step, which is what cascades would remove; sampled with **normal-offset bias + 3x3 PCF** - the normal offset removes acne without the large depth bias that used to erase thin-caster shadows, and the 3x3 grid of hardware comparison taps smooths the edges; a LINEAR depth-compare sampler = hardware percentage-closer filtering); every caster is rasterized **two-sided** in the depth pass (a single-sided wall plane whose only face points into the room is back-facing from the sun, so a back-face-culled depth pass would never record it and the sunlight would pour through a wall the camera sees as solid - the "wall behind the camera is transparent" leak); masked (`alphaMode MASK`) casters are alpha-tested **inside the depth pass** - shadow.frag reads the same material record and texture array as pbr.frag and takes one uniform early-out branch per draw - so foliage / curtains / grates cast cut-out shadows instead of none (BLEND leaves still cast none, since a depth-only pass cannot blend one); enabled over the imported scene's bounds. The pass's depth bias is dynamic state, tunable live from the debug GUI (slope factor for acne on angled surfaces, constant factor for a fixed push). Shadow casters are selected per scene size: scenes up to 1500 leaves render **every leaf** into the shadow map (a wall arbitrarily far up-light still throws its parallel shadow column into the view, so a finite camera-margin would leak sunlight through interiors like Sponza); heavier scenes draw the camera-visible leaves plus those the BVH reports inside the **shadow frustum** itself - which already contains the off-screen casters - so the depth pass stays cheap (the runtime logs once when that switch happens). The fit itself is refitted only when the camera matrices or the scene actually changed (a cached fit, since walking every leaf costs tens of thousands of transforms per frame), instanced leaves contribute their **exact** bounds - the union of the source geometry's local AABB over each of their instance matrices, mirroring what the vertex shader does - and only a caster that is neither bounded nor instanced falls back to the scene sphere, whose light-space xy is still clamped to the camera footprint so it cannot coarsen the map.
...
- **Startup configuration (`app_config` module)**: TOML config (`config.toml`, `--config <path>` override) merged with argv, covering model / instancing grid, resource paths, window/render settings (size, title, vsync, MSAA - 0 = auto, 1 = off, N = count, clear color, skybox/shadow/fxaa/gpu-timings toggles, the G-buffer debug view and its channel) and IBL resolutions. See `config.example.toml`.
