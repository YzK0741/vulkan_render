...
void main() {
    ...
    // ... but the material byte DOES: flags bit7 is the PAINTED bit - the one that says "draw this from its
    // albedo, not from the lighting stack" - and it is 7 precisely so that it fits in this byte (see
    // material_record). Stored as k/255, so *255 recovers the byte exactly.
    si.painted_mask = (uint(round(material.a * 255.0)) & 128u) != 0u ? 1.0 : 0.0;
