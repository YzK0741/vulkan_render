...
| `motion_dd.py <sweep_on> <static_on> <sweep_off> <static_off>` | does the glossy reflection survive camera motion? Four arms - a swept and a converged STATIC capture at the sweep's END pose, each with the lobe on and off - make it a difference of differences, so the still-versus-moving pose change cancels and what is left is the reflection's own motion loss. Reports each arm and `mean\|DD\|`: 0.4029 before this renderer capped a smooth pixel's accumulation by roughness, 0.3492 after. **The pose rules ARE the instrument**: the static pair has to be at `base + sweep * frames`, and `capture.ps1`'s own `-Camera` default is Sponza's pose, so a static arm taken at the base pose gives a plausible number rather than an obviously wrong one - the script's docstring carries the two commands and where the base pose is read from |
